...
def intrinsic_features(
    ...
    X_train = features_tensor[train_mask_sampled].to(device_decoder)
    y_train = y_tensor[train_mask_sampled].to(device_decoder)
    X_test = features_tensor[test_mask.bool()].to(device_decoder)
    y_test = y_tensor[test_mask.bool()].to(device_decoder)
...
def node2vec_features(
    ...
    x_test = x[test_mask.bool()].to(device_decoder).squeeze()
    y_train = y_tensor[train_mask_sampled].to(device_decoder).squeeze()
    y_test = ntw_torch.y[test_mask.bool()].to(device_decoder).squeeze()
