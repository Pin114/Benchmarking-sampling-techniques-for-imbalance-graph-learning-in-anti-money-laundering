                ...
                try:
                    if method == "intrinsic":
                        ap_score, y_pred_probs, y_true = intrinsic_features_with_predictions(
                            ...
                            lr=args.lr, n_epochs_decoder=100, ratio=ratio, sampling=sampling, loss=args.loss, loss_kwargs=loss_kwargs
                        )
                        ...
                        if sampling in ["none", "random_undersample"]:
                            ap_score, y_pred_probs, y_true = GNN_features_with_predictions(
                                ...
                                checkpoint_path=unique_checkpoint_path, monitor='val_ap', ratio=ratio, sampling=sampling, loss=args.loss, loss_kwargs=loss_kwargs
                            )
                        else:
                            ap_score, y_pred_probs, y_true = GNN_features_graphsmote_with_predictions(
                                ...
                                monitor='val_ap', ratio=ratio, loss=args.loss, loss_kwargs=loss_kwargs,
                                gatsmote_k_neighbors=args.gatsmote_k_neighbors, gatsmote_attention_heads=args.gatsmote_attention_heads,
                                gatsmote_edge_threshold=args.gatsmote_edge_threshold, gatsmote_homophily_weight=args.gatsmote_homophily_weight,
                                gatsmote_use_predicted_labels_for_homophily=args.gatsmote_use_predicted_labels
                            )
