...
def resolve_params_path(method: str, ntw_name: str, sampling: str = None):
    """Return the first existing params file path for the given method.

    Tries these candidates in order and returns the first that exists:
      - res/{method}_params_{ntw_name}_supervised_{sampling}.txt
      - res/{method}_params_{ntw_name}_{sampling}.txt
      - res/{method}_params_{ntw_name}.txt

    If none exist, returns the second candidate (so caller gets a clear FileNotFoundError).
    """
    samp_tag = '' if not sampling or sampling == 'none' else f'_{sampling}'
    candidates = [
        f"res/{method}_params_{ntw_name}_supervised{samp_tag}.txt",
        f"res/{method}_params_{ntw_name}{samp_tag}.txt",
        f"res/{method}_params_{ntw_name}.txt",
    ]
    for p in candidates:
        if os.path.exists(p):
            return p
...
if __name__ == "__main__":
    ...
    if 'intrinsic' in to_test:
        ...
        params_path = resolve_params_path("intrinsic", ntw_name)
        with open(params_path, "r") as f:
            params = f.readlines()
        string_dict = params[0].strip()
        try:
            param_dict = ast.literal_eval(string_dict)
        except Exception:
    ...
    if 'positional' in to_test:
        ...
        params_path = resolve_params_path("positional", ntw_name)
        with open(params_path, "r") as f:
            params = f.readlines()
        string_dict = params[0].strip()
        try:
    ...
    if 'deepwalk' in to_test:
        print("Deepwalk")
        params_path = resolve_params_path("deepwalk", ntw_name)
        with open(params_path, "r") as f:
            params = f.readlines()
        string_dict = params[0].strip()
        try:
    ...
    if 'node2vec' in to_test:
        ## node2vec
        print("Node2vec")
        params_path = resolve_params_path("node2vec", ntw_name)
        with open(params_path, "r") as f:
            params = f.readlines()
        string_dict = params[0].strip()
        try:
    ...
    if 'gcn' in to_test:
        ## GCN
        print("GCN")
        params_path = resolve_params_path("gcn", ntw_name)
        with open(params_path, "r") as f:
            params = f.readlines()
        string_dict = params[0].strip()
        try:
    ...
    if 'sage' in to_test:
        #GraphSAGE
        print("GraphSAGE")
        params_path = resolve_params_path("sage", ntw_name)
        with open(params_path, "r") as f:
            params = f.readlines()
        string_dict = params[0].strip()
        try:
    ...
    if 'gat' in to_test:
        # GAT
        print("GAT")
        params_path = resolve_params_path("gat", ntw_name)
        with open(params_path, "r") as f:
            params = f.readlines()
        string_dict = params[0].strip()
        try:
    ...
    if 'gin' in to_test:
        # GIN
        print("GIN")
        params_path = resolve_params_path("gin", ntw_name)
        with open(params_path, "r") as f:
            params = f.readlines()
        string_dict = params[0].strip()
        try:
