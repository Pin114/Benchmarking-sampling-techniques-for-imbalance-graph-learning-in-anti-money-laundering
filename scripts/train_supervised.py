    ...
    for ratio in test_ratios:
        ...
        for method in to_train:
            ...
            for sampling in sampling_techniques_for_method:
                ...
                # === Dynamic checkpoint path generation ===
                            ...
                            if method == "intrinsic":
                                if sampling == "none":
                                    ap_loss, f1_loss = intrinsic_features(
                                        ntw, train_mask_sampled, test_mask, n_layers_decoder=2, hidden_dim_decoder=16, lr=0.05, n_epochs_decoder=100, percentile_q=thresh
                                    )
                                elif sampling == "random_undersample":
                                    ap_loss, f1_loss = intrinsic_features(
                                        ntw, train_mask_sampled, test_mask, n_layers_decoder=2, hidden_dim_decoder=16, lr=0.05, n_epochs_decoder=100, percentile_q=thresh
                                    ...
                                    ap_loss, f1_loss = intrinsic_features_smote(
                                        ntw, train_mask_sampled, test_mask, n_layers_decoder=2, hidden_dim_decoder=16, lr=0.05, n_epochs_decoder=100, k_neighbors=5, random_state=42, percentile_q=thresh
                                    )
                            elif method == "positional":
                                if sampling == "none":
                                    ap_loss, f1_loss = positional_features(
                                        ntw, train_mask_sampled, test_mask, alpha_pr=0.5, alpha_ppr=0, n_epochs_decoder=50, lr=0.05, fraud_dict_test=fraud_dict, n_layers_decoder=2, hidden_dim_decoder=16, ntw_name=ntw_name+"_train", percentile_q=thresh
                                    )
                                elif sampling == "random_undersample":
                                    ap_loss, f1_loss = positional_features(
                                        ntw, train_mask_sampled, test_mask, alpha_pr=0.5, alpha_ppr=0, n_epochs_decoder=50, lr=0.05, fraud_dict_test=fraud_dict, n_layers_decoder=2, hidden_dim_decoder=16, ntw_name=ntw_name+"_train", percentile_q=thresh
                                    ...
                                    ap_loss, f1_loss = positional_features_smote(
                                        ntw, train_mask_sampled, test_mask, alpha_pr=0.5, alpha_ppr=0, n_epochs_decoder=50, lr=0.05, fraud_dict_test=fraud_dict, n_layers_decoder=2, hidden_dim_decoder=16, ntw_name=ntw_name+"_train", k_neighbors=5, random_state=42, percentile_q=thresh
                                    )
                            elif method == "deepwalk":
                                ap_loss, f1_loss = node2vec_features(
                                    ntw_torch, train_mask_sampled, test_mask, embedding_dim=16, walk_length=3, context_size=2, walks_per_node=1, num_negative_samples=2, p=1, q=1, lr=0.05, n_epochs=30, n_epochs_decoder=30, use_torch=True, percentile_q=thresh
                                )
                            elif method == "node2vec":
                                ap_loss, f1_loss = node2vec_features(
                                    ntw_torch, train_mask_sampled, test_mask, embedding_dim=16, walk_length=3, context_size=2, walks_per_node=1, num_negative_samples=2, p=1.5, q=1.0, lr=0.05, n_epochs=20, n_epochs_decoder=20, ntw_nx=ntw.get_network_nx(), use_torch=True, percentile_q=thresh
                                )
                            elif method == "gcn":
                                if sampling in ["none", "random_undersample"]:
                                    model_gcn = GCN(
                                        edge_index=edge_index, num_features=num_features, hidden_dim=64, embedding_dim=32, output_dim=output_dim, n_layers=2, dropout_rate=0.3
                                    ).to(device)
                                    ap_loss, f1_loss = GNN_features(
                                        ntw_torch, model_gcn, lr=0.05, n_epochs=50, train_mask=train_mask_sampled, val_mask=val_mask, test_mask=test_mask, percentile_q=thresh,
                                        patience=10, checkpoint_path=unique_checkpoint_path, monitor='val_ap'
                                    )
                                    ...
                                    ap_loss, f1_loss = GNN_features_graphsmote(
                                        ntw_torch, model_gcn, lr=0.05, n_epochs=50, train_mask=train_mask_sampled, val_mask=val_mask, test_mask=test_mask, k_neighbors=5, random_state=42, percentile_q=thresh, use_reweighted=(sampling == "reweighted_graph_smote"),
                                        patience=10, checkpoint_path=unique_checkpoint_path, monitor='val_ap'
                                    )
                            elif method == "sage":
                                if sampling in ["none", "random_undersample"]:
                                    model_sage = GraphSAGE(
                                        edge_index=edge_index, num_features=num_features, hidden_dim=64, embedding_dim=32, output_dim=output_dim, n_layers=2, dropout_rate=0.3, sage_aggr="mean"
                                    ).to(device)
                                    ap_loss, f1_loss = GNN_features(
                                        ntw_torch, model_sage, lr=0.05, n_epochs=50, train_mask=train_mask_sampled, val_mask=val_mask, test_mask=test_mask, percentile_q=thresh,
                                        patience=10, checkpoint_path=unique_checkpoint_path, monitor='val_ap'
                                    )
                                else: # graph_smote / graph_ensemble_smote / reweighted_graph_smote
                                    model_sage = GraphSAGE(
                                    ...
                                    ap_loss, f1_loss = GNN_features_graphsmote(
                                        ntw_torch, model_sage, lr=0.05, n_epochs=50, train_mask=train_mask_sampled, val_mask=val_mask, test_mask=test_mask, k_neighbors=5, random_state=42, percentile_q=thresh, use_reweighted=(sampling == "reweighted_graph_smote"),
                                        patience=10, checkpoint_path=unique_checkpoint_path, monitor='val_ap'
                                    )
                            elif method == "gat":
                                ...
                                if sampling in ["none", "random_undersample"]:
                                    model_gat = GAT(
                                        num_features=num_features, hidden_dim=gat_hidden_dim, embedding_dim=gat_embedding_dim, output_dim=output_dim, n_layers=2, heads=gat_heads, dropout_rate=gat_dropout
                                    ).to(device)
                                    ap_loss, f1_loss = GNN_features(
                                        ntw_torch, model_gat, lr=0.05, n_epochs=gat_epochs, train_mask=train_mask_sampled, val_mask=val_mask, test_mask=test_mask, percentile_q=thresh,
                                        patience=10, checkpoint_path=unique_checkpoint_path, monitor='val_ap'
                                    )
                                else: # graph_smote / graph_ensemble_smote / reweighted_graph_smote
                                    model_gat = GAT(
                                    ...
                                    ap_loss, f1_loss = GNN_features_graphsmote(
                                        ntw_torch, model_gat, lr=0.05, n_epochs=gat_epochs, train_mask=train_mask_sampled, val_mask=val_mask, test_mask=test_mask, k_neighbors=5, random_state=42, percentile_q=thresh, use_reweighted=(sampling == "reweighted_graph_smote"),
                                        patience=10, checkpoint_path=unique_checkpoint_path, monitor='val_ap'
                                    )
                            elif method == "gin":
                                if sampling in ["none", "random_undersample"]:
                                    model_gin = GIN(
                                        num_features=num_features, hidden_dim=64, embedding_dim=32, output_dim=output_dim, n_layers=2, dropout_rate=0.3
                                    ).to(device)
                                    ap_loss, f1_loss = GNN_features(
                                        ntw_torch, model_gin, lr=0.05, n_epochs=50, train_mask=train_mask_sampled, val_mask=val_mask, test_mask=test_mask, percentile_q=thresh,
                                        patience=10, checkpoint_path=unique_checkpoint_path, monitor='val_ap'
                                    )
                                else: # graph_smote / graph_ensemble_smote / reweighted_graph_smote
                                    model_gin = GIN(
                                    ...
                                    ap_loss, f1_loss = GNN_features_graphsmote(
                                        ntw_torch, model_gin, lr=0.05, n_epochs=50, train_mask=train_mask_sampled, val_mask=val_mask, test_mask=test_mask, k_neighbors=5, random_state=42, percentile_q=thresh, use_reweighted=(sampling == "reweighted_graph_smote"),
                                        patience=10, checkpoint_path=unique_checkpoint_path, monitor='val_ap'
                                    )
